...
{
    [Route("jack")]
    public class Items_JackController : Controller
    {
        private readonly GameInvContext _context;
        public Items_JackController(GameInvContext context) => _context = context;
        ...
        // GET /jack/details/123
        [HttpGet("details/{id:int}")]
        public async Task<IActionResult> Details(int id)
        {
            var item = await _context.Inventory.FirstOrDefaultAsync(m => m.ItemId == id);
            if (item == null) return NotFound();
            return View(item); // Views/Items_Jack/Details.cshtml
        }

        // GET /jack/create
        [HttpGet("create")]
        public IActionResult Create() => View(); // Views/Items_Jack/Create.cshtml
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            if (item == null) return NotFound();
            return View(item); // Views/Items_Jack/Edit.cshtml
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            var item = await _context.Inventory.FirstOrDefaultAsync(m => m.ItemId == id);
            if (item == null) return NotFound();
            return View(item); // Views/Items_Jack/Delete.cshtml
        }

        // POST /jack/delete/123
