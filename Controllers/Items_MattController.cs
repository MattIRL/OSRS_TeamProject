...
{
    [Route("matt")]
    public class Items_MattController : Controller
    {
        private readonly GameInvContext _context;
        public Items_MattController(GameInvContext context) => _context = context;
        ...
        // GET /matt/details/123
        [HttpGet("details/{id:int}")]
        public async Task<IActionResult> Details(int id)
        {
            var item = await _context.Inventory.FirstOrDefaultAsync(m => m.ItemId == id);
            if (item == null) return NotFound();
            return View(item); // Views/Items_Matt/Details.cshtml
        }

        // GET /matt/create
        [HttpGet("create")]
        public IActionResult Create() => View(); // Views/Items_Matt/Create.cshtml
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            if (item == null) return NotFound();
            return View(item); // Views/Items_Matt/Edit.cshtml
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            var item = await _context.Inventory.FirstOrDefaultAsync(m => m.ItemId == id);
            if (item == null) return NotFound();
            return View(item); // Views/Items_Matt/Delete.cshtml
        }

        // POST /matt/delete/123
